...
class NPlus1GramStats(EquivalEqual):
    ...
    def __init__(self,CDsRClosed,Order=True,U1Tails=None,U2Grams=None,CorpusID=None,SentCnt=None):
        ...
        if CDsRClosed and type(list(CDsRClosed.values())[0]).__name__=='int':
            CDsR={ ():{U1:PD} for (U1,PD) in CDsRClosed.items() }
        else:
    ...
    def check_length(self,CDsR):
        ...
        if not all(len(U1) == FstElLen for U1 in CDsR.keys()):
            sys.exit('inconsistent length of ngram')
        else:
            return FstElLen
    ...
    def generate_filteredstats(self,Threshs):
        ...
        for Cntr,(Unit1,PostDist) in enumerate(self.conddists.items()):
            if Unit1Cnt>10000 and Cntr!=0 and Cntr%5000==0: print(str(Cntr)+' unit1s done')
...
#        FilteredTokenCnt=FilteredTypeCnt=0
        ...
        for Unit2,Unit2Occ in PostDist.evtocc.items():
            if Unit2 in self.u2grams.evtocc.keys():
                ...
                ## this is where the bigram stats are computed 
                ##
